...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (pastedText.length > 50 && pastedText.includes("\n")) {
      ...
      if (hasImports || hasFunctions || hasCodeStructure) {
        ...
        if (fileName) {
          addFileAttachment(fileName, Date.now().toString(), pastedText);
          return;
        }
      }
    }
  ...
  const addCurrentFileAsContext = () => {
    if (activeFileName && activeFileContent && activeFileId) {
      if(attachments.find(f => f.fileId === activeFileId)) return
      addFileAttachment(activeFileName, activeFileId, activeFileContent);
    }
  };
